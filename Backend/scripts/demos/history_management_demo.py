...
def run_demo():
    ...
    naive_tokens = estimate_tokens(SYSTEM_PROMPT)
    for turn in CONVERSATION_TURNS:
        naive_tokens += estimate_tokens(turn["user"])
        naive_tokens += estimate_tokens(turn["assistant"])
    ...
    for turn_idx, turn_data in enumerate(CONVERSATION_TURNS, 1):
        ...
        user_tokens = estimate_tokens(turn_data["user"])
        ...
        assistant_tokens = estimate_tokens(turn_data["assistant"])
