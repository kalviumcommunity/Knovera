...
def tag_chunk_metadata(
    ...
    """Tag a chunk with uniform metadata."""
    meta = _default_tagger.create_metadata(
        source=source_file,
        chunk_index=chunk_index,
        char_start=0,
        char_end=len(chunk.get("text", "")),
        file_type=Path(source_file).suffix,
        doc_title=doc_title or source_file,
        extra={"doc_id": doc_id, "token_count": chunk.get("token_count", 0)}
    )
    meta["doc_id"] = doc_id
