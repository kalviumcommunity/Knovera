...
def check_embedding_quality(
    ...
    """Check vector validity, dimensions, and cosine similarity."""
    list_a = vector_a.tolist() if hasattr(vector_a, "tolist") else list(vector_a)
    list_b = vector_b.tolist() if hasattr(vector_b, "tolist") else list(vector_b)
    
    sim = cosine_similarity(list_a, list_b)
    return {
        "is_valid": len(list_a) == len(list_b) and len(list_a) > 0,
        "dimension": len(list_a),
        "cosine_similarity": round(sim, 4),
        "meets_threshold": sim >= min_similarity
    }
