...
_default_loader = DocumentLoader(verbose=False)


def load_document(path) -> str:
    """Load and extract text from a file path."""
    p = Path(path)
    return _default_loader.load_text(p)


def extract_metadata_from_filename(filename: str) -> Dict[str, Any]:
    """Extract metadata details from filename."""
    p = Path(filename)
