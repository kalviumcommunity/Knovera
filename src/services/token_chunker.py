...
def estimate_tokens(text: str) -> int:
    ...
    return _default_chunker.count_tokens(text)
...
def chunk_text_by_tokens(
    ...
    """Chunk text into token-bounded dictionaries."""
    if not text or not text.strip():
        return []
    raw_chunks = _default_chunker.token_chunks(text, size=chunk_size, overlap=chunk_overlap)
    return [
        {
            "text": chunk_str,
            "token_count": _default_chunker.count_tokens(chunk_str),
            "chunk_index": idx
        }
